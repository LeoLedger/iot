...
<template>
    <layout-box>
        <layout-filter
            ...
            :filter-button="checkFeature(['ty.iot.approval.stocktaking.search'])"
            class="project"
            @search="handleFilter"
        >
            <template slot="button">
                <el-button
                    v-waves
                    v-if="checkFeature(['ty.iot.approval.stocktaking.export'])"
                    size="mini"
                    class="filter-button"
                    type="warning"
                ...
                <template v-if="isEdit">
                    <el-button
                        v-waves
                        v-if="checkFeature(['ty.iot.approval.stocktaking.create'])"
                        size="mini"
                        class="filter-button"
                        type="success"
                        icon="el-icon-plus"
            ...
            <template slot="filter-popup">
                <layout-filter-item label="所属仓库">
                    <WarehouseCascader
                        ...
                        place-key="shippingSpaceId"
                        placeholder="所属仓库"
                        clearable
                    />
                </layout-filter-item>
                <layout-filter-item label="计划盘点时间">
                    <el-date-picker
                        v-model="searchParam.planDate"
                        :default-time="['00:00:00', '23:59:59']"
                        type="daterange"
                        format="yyyy-MM-dd"
                        value-format="yyyy-MM-dd HH:mm:ss"
                ...
                tableData: {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            minWidth: 200,
                            formatter: (row) => {
                                return `${row.warehousesName || '- -'} | ${row.shippingSpaceName || '- -'}`
                            }
                        },
                        {
                            type: 'date',
