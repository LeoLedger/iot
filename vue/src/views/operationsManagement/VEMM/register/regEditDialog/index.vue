<!--
* @description 登记维修单 - 编辑模式 - 分开写是因为创建和编辑差别大 不好兼容
* @author 望浩然
...
<script>
    import BaseForm from '../regDialog/BaseForm'
        ...
        methods: {
            ...
            handleSubmit() {
                ...
                this.$refs.deviceList.submit().then(() => {
                    ...
                    this.$refs.BaseForm.submit((data) => {
                        this.loading = true
                        const postData = { ...this.formData, ...data, listFaultCu: this.listFaultCu }
            ...
            handleSelectChange(list, showMessage = true) {
                ...
                if (list && list.length) {
                    ...
                    list.map(item => {
                        if (item.guidId) {
                            ...
                            let hasOne = false
                            defaultListFaultCu.map((_item, _index) => {
                                if (_item.deviceId === newItem.deviceId) {
                                    defaultListFaultCu.splice(_index, 1)
                                }
                            })
                ...
                if (newListFaultCu.length) {
                    if (showMessage) this.$message({ type: 'success', message: '成功导入' + newListFaultCu.length + '台设备!' })
                    // 合并旧列表和新选择的列表
