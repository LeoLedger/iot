...
<template>
    <div class="inventory-container">
        <layout-template
            ...
            :filter-button="checkFeature(['ty.iot.otherProducts.search'])"
            @search="handleSearch"
        >
            <template slot="left">
            ...
            <template slot="button">
                <el-button
                    v-waves
                    v-prereclick
                    v-if="checkFeature(['ty.iot.otherProducts.export'])"
                    type="warning"
                    size="mini"
                    class="filter-item"
                ...
                <ImportButton
                    v-if="['事业部仓库', '客服部门仓库'].includes(currentWarehouseName)"
                    :before-upload="hanldeBeforeUpload"
                    :download-method="handleDownloadTemplate"
                    :upload-data="importDialog.params"
                    type="primary"
                    action="device/costCenter/upload"
                    title="修改成本中心"
                    template-name="修改成本中心模板.xlsx"
        ...
        <!-- 设备分配窗口 -->
        <backup-dialog
            v-if="backupDialog.visible"
            :visible="backupDialog.visible"
            :row-data="backupDialog.rowData"
            @close="handleDialogEvent('backupDialog')"
        />
        <!-- 入库申请单窗口 -->
        <StoragePutDialog
        ...
        <!-- 设备报损申请单 -->
        <DeviceDamagedDialog
            v-if="damagedDialog.visible"
            :visible.sync="damagedDialog.visible"
            :extend-params="{ }"
        />
        <!-- 设备挂起窗口 -->
        <HangupDialog
            v-if="setOffDialog.visible"
            :visible.sync="setOffDialog.visible"
            :extend-params="{ }"
            @success="getList"
        />
    </div>
...
<script>
    ...
    import BackupDialog from '@/views/inventoryManagement/Backup/BackupDialog'
    import StoragePutDialog from '@/views/businessDocuments/components/StoragePut/components/UpdateDialog'
    import DeviceDamagedDialog from '@/views/businessDocuments/components/DeviceDamaged/components/UpdateDialog'
    import HangupDialog from '@/views/inventoryManagement/SetOff/index'
    ...
    export default {
        ...
        watch: {
            ...
            'searchParamsLock.warehousesSpaceId'() {
                this.getList()
            }
        },
        mounted() {
        ...
        methods: {
            ...
            init() {
                // 当前页面的产品类型
                this.productType = 'B16C3151-11D2-4364-8F45-63EFFFFCF424'
                // 获取仓库列表
            ...
            async getList() {
                ...
                // 必须有仓库ID和仓位ID才查询,否则取消,不然会报错
                if (params.warehousesId && params.warehousesSpaceId) {
                    ...
                    if (res.success) {
                        ...
                        item.productTypeData = res.data.total
                    }
                }
                setTimeout(() => {
            ...
            async getWarehousesList() {
                ...
                if (res.success) {
                    ...
                    if (this.warehousesList && this.warehousesList.length) {
                        ...
                        this.currentWarehouseName = this.warehousesList[0].name
                        // 提前结束仓库的加载效果
            ...
            onClickChangeWareHouse(item) {
                ...
                this.currentWarehouseName = item.name // 当前仓库名称
                // 切换仓库时重新加载仓位
                this.getWarehouseShippingSpace(item.guidId)
            },
            handleDownloadTemplate() {
                warehousesApi.downloadTemplate({})
