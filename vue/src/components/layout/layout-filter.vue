<template>
    <layout-col :height="height" auto>
        <layout-col auto>
            <layout-row>
                <slot name="filter-top" />
            </layout-row>
            <layout-row v-if="showToolbar">
                <layout-col not-flex>
                ...
                <layout-col ref="layout-filter" auto not-flex>
                    <div v-if="filterButton" class="layout-filter" style="text-align: right;">
                        <slot name="filter"/>
...
<script>
    export default {
        ...
        methods: {
            ...
            updatePopupVisible() {
                this.filterPopupVisible = this.filterButton && this.$slots['filter-popup']
            },
            handleSearch() {
            ...
            handleResetFilter() {
                ...
                var defaultModel = this.filter.model
                Object.keys(filterModel).forEach(key => {
                    if (this.lockedProps.indexOf(key) === -1) {
                        data[key] = key in defaultModel ? defaultModel[key] : void 0
                    }
                })
                this.$emit('update:filterModel', {
                    ...this.filterModel,
                    ...data
                })
                this.$emit('search')
