...
import { getUserNotCallInPrivilegeList, callInPrivilege } from '@/api/user/index'
...
export default {
  ...
  created() {
    ...
    this.title = this.user.deptName + '(' + this.user.userName + ')' + '权限调入'
    this.nextTickWrap()
    this.queryList()
  },
  methods: {
    ...
    queryList() {
      ...
      getUserNotCallInPrivilegeList(params).then(res => {
        let data = null
        if (res.data.success) {
