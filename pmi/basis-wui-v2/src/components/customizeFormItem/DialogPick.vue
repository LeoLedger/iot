    ...
    export default {
        ...
        computed: {
            ...
            pickValue() {
                ...
                let props = this.dataKey.map(v => v)
                if (multiple && Array.isArray(value)) {
                    return value.map(item => {
                        let data = {}
                        props.forEach((key, index) => {
                            data[key] = item[this.formKey[index]]
                        })
                        return data
                    })
                } else if (!isEmpty(value)) {
                    let data = {
                        [labelKey]: formData[prop]
                    }
                    props.forEach((key, index) => {
            ...
            viewValue() {
                var value = this.formData[this.insideParams.prop]
                if (this.insideParams.multiple) {
                    if (Array.isArray(value)) {
                        return value.map(item => item[this.insideParams.labelKey])
                    }
                    return []
                }
                return value
                ...
                if (this.insideParams.multiple) {
                    ...
                    var value = list.map(item => {
                        ...
                        this.formKey.forEach((key, index) => {
                            data[key] = item[this.dataKey[index]]
                        })
                        return data
                    })
                    this.$set(this.formData, this.insideParams.prop, value)
